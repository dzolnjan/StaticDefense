...
namespace StaticDefence.Core
{
    public class Map
    {
        ...
        public void AddRoad(float distance, Directions direction)
        {
            if (Roads.Count == 0)
            {
                if (direction == Directions.Right)
                    Roads.Add(new Road(Directions.Right) { Left = Start.X, Top = Start.Y, Distance = distance, Thickness = RoadThickness });
                else if (direction == Directions.Left)
                    Roads.Add(new Road(Directions.Left) { Left = Start.X - distance, Top = Start.Y, Distance = distance, Thickness = RoadThickness });
                else if (direction == Directions.Down)
                    Roads.Add(new Road(Directions.Down) { Left = Start.X, Top = Start.Y, Distance = distance, Thickness = RoadThickness });
                else if (direction == Directions.Up)
                    Roads.Add(new Road(Directions.Up) { Left = Start.X, Top = Start.Y - distance, Distance = distance, Thickness = RoadThickness });
            }
            else
            {
                ...
                if (direction == Directions.Right)
                {
                    if (last.Direction == Directions.Down)
                        Roads.Add(new Road(direction) { Left = last.Left + RoadThickness, Top = last.Top + last.Distance, Distance = distance, Thickness = RoadThickness });
                    if (last.Direction == Directions.Up)
                        Roads.Add(new Road(direction) { Left = last.Left + RoadThickness, Top = last.Top - RoadThickness, Distance = distance, Thickness = RoadThickness });
                }
                if (direction == Directions.Left)
                {
                    if (last.Direction == Directions.Down)
                        Roads.Add(new Road(direction) { Left = last.Left - last.Distance, Top = last.Top + last.Distance, Distance = distance, Thickness = RoadThickness });
                    if (last.Direction == Directions.Up)
                        Roads.Add(new Road(direction) { Left = last.Left - last.Distance, Top = last.Top - RoadThickness, Distance = distance, Thickness = RoadThickness });
                }
                ...
                {
                    if (last.Direction == Directions.Right)
                        Roads.Add(new Road(direction) { Left = last.Left + last.Distance, Top = last.Top - distance, Distance = distance, Thickness = RoadThickness });
                    if (last.Direction == Directions.Left)
                        Roads.Add(new Road(direction) { Left = last.Left - RoadThickness, Top = last.Top - distance, Distance = distance, Thickness = RoadThickness });
                }
            }
        ...
        private void AddJunction(Directions direction)
        {
            ...
            {
                if (last.Direction == Directions.Up)
                    Junctions.Add(new Juntion(320) { Left = last.Left, Top = last.Top - RoadThickness, Width = RoadThickness });
                else if (last.Direction == Directions.Down)
                    Junctions.Add(new Juntion(40) { Left = last.Left, Top = last.Top + last.Distance, Width = RoadThickness });
                //else
            ...
            {
                if (last.Direction == Directions.Up)
                    Junctions.Add(new Juntion(220) { Left = last.Left, Top = last.Top - RoadThickness, Width = RoadThickness });
                else if (last.Direction == Directions.Down)
                    Junctions.Add(new Juntion(140) { Left = last.Left, Top = last.Top + last.Distance, Width = RoadThickness });
                //else
                //    throw "Cant add junction to " + direction.ToString() + "
            }
            else if (direction == Directions.Up)
            {
                if (last.Direction == Directions.Right)
                    Junctions.Add(new Juntion(310) { Left = last.Left + last.Distance, Top = last.Top, Width = RoadThickness });
                else if (last.Direction == Directions.Left)
                    Junctions.Add(new Juntion(140) { Left = last.Left + last.Distance, Top = last.Top, Width = RoadThickness });
                //else
                //    throw "Cant add junction to " + direction.ToString() + "
            }
            else if (direction == Directions.Down)
            {
                if (last.Direction == Directions.Right)
                    Junctions.Add(new Juntion(50) { Left = last.Left + last.Distance, Top = last.Top, Width = RoadThickness });
                else if (last.Direction == Directions.Left)
                    Junctions.Add(new Juntion(130) { Left = last.Left - RoadThickness, Top = last.Top, Width = RoadThickness });
                //else
                //    throw "Cant add junction to " + direction.ToString() + "
            }
        }
